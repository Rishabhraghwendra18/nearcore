...
/// EdgeListToVerify contains list of Edges, and it's associated with a connected peer.
/// Check signatures of all edges in `EdgeListToVerify` and if some signature is not valid,
...
impl Handler<EdgeListToValidate> for EdgeVerifierActor {
    type Result = bool;

    #[perf]
    fn handle(&mut self, msg: EdgeListToValidate, _ctx: &mut Self::Context) -> Self::Result {
